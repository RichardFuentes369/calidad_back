...
export class ZonaService {
  ...
  async create(createZonaDto: CreateZonaDto) {
    // const encontrarCorreo = await this.findUsernameEmail(createZonaDto.email)

    // if(encontrarCorreo) throw new NotFoundException(`
    //   Este correo ${createZonaDto.email}, ya esta registrado en nuestra base de datos
    // `)
  ...
  async update(id: number, updateZonaDto: UpdateZonaDto) {
    ...
    //     let concidencia = await this.zonaRepository.findOne({
